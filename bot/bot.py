...
def cmd_myip(message):
   ...
   if valid_id in allowgroups() or valid_id2 in allowusers():
     if re.findall(r'[\d]+\.[\d]+\.[\d]+\.[\d]+', message.text):
        ...
        cmdoutput = subprocess.Popen('/var/cld/bot/bin/myip_add '+str(message.from_user.id)+' '+str(message.from_user.username)+' '+str(myip), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def cmd_enableip(message):
   ...
   if valid_id in allowusers():
     if re.findall(r'([\d]+\.[\d]+\.[\d]+\.[\d]+)\s(.+)', message.text):
        ...
        cmdoutput = subprocess.Popen('/var/cld/bot/bin/enableip_add '+str(enable_ip)+' '+str(enable_cmnt), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        bot.send_message(message.chat.id, cmdoutput.communicate(), parse_mode='Markdown')
     else:
