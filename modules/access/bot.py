...
def cmd_myip(message):
  ...
  if re.findall(r'[\d]+\.[\d]+\.[\d]+\.[\d]+', message.text):
    ...
    cmdoutput = bash('sudo -u '+user+' sudo FROM=BOT /var/cld/modules/access/bin/cld-myipbot '+vld(message.from_user.id)+' '+vld(user)+'@'+vld(message.from_user.username)+' '+vld(myip))
...
def cmd_myvpn(message):
  ...
  try:
      ...
      pass
  cmdoutput = bash('sudo -u '+user+' sudo FROM=BOT /var/cld/modules/access/bin/cld-myvpnbot '+vld(message.from_user.id)+' '+vld(user)+'@'+vld(message.from_user.username)+' '+cmd_args)
  last_message = bot.send_message(message.chat.id, cmdoutput, parse_mode='Markdown', disable_web_page_preview='true')
