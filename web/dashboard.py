...
socketio = SocketIO(app, cors_allowed_origins='https://dev-panel.classicdeploy.com', threading=greenlet)
  ...
  if 'username' in session:
    ...
    if child_pid == 0:
      ...
      while subprocpid == '' and count != 30:
        ...
        count+=1
        print('ps axf -o pid,command | grep -v grep | egrep -A1 "^\ +?'+str(child_pid)+' " | cut -d " " -f 1 | tail -1 | tr -d "\n"', flush=True)
        time.sleep(0.1)
