...
def read_and_forward_pty_output(socketid):
    ...
    while True:
        socketio.sleep(0.01)
        if app.config['socketid']:
            timeout_sec = 0
            (data_ready, _, _) = select.select([app.config['socketid']], [], [], timeout_sec)
            if data_ready:
                output = os.read(app.config['socketid'], max_read_bytes).decode()
...
def pty_input(data):
    ...
    if app.config['socketid']:
        os.write(app.config['socketid'], data["input"].encode())
...
def resize(data):
    ...
    if app.config['socketid']:
        set_winsize(app.config['socketid'], data["rows"], data["cols"])
